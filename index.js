...
`}function P(){l.galleryList.innerHTML=""}function h(){l.loader.classList.remove("hidden")}function c(){l.loader.classList.add("hidden")}function v(){l.btnLoadMore.classList.remove("hidden")}function g(){l.btnLoadMore.classList.add("hidden")}const d={formElem:document.querySelector(".form"),btnLoadMore:document.querySelector(".btn-loadMore"),galleryList:document.querySelector(".gallery"),targetElem:document.querySelector(".js-target")},a={query:"",page:1,total:100,perPage:15};//! Послуховувач події на форму, для збирання даних з поля введеня користувачем
d.formElem.addEventListener("submit",o=>{if(o.preventDefault(),a.query=o.target.elements.search.value.trim(),a.page=1,P(),!a.query){u.error({title:"Error",message:"Please enter a search term!",position:"topRight"});return}h(),g(),p(a.query,a.page,a.perPage).then(t=>{c(),t.hits.length===0&&(u.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight",timeout:1e4}),g()),f(t.hits),a.total=t.totalHits,y(),c()}),d.formElem.reset()});d.btnLoadMore.addEventListener("click",()=>{a.page+=1,y(),h(),p(a.query,a.page).then(o=>{f(o.hits),c()}),w()});function y(){const t=Math.ceil(a.total/15);a.page>=t?(g(),c(),u.info({title:"Info",message:"We're sorry, but you've reached the end of search results.",position:"topRight",timeout:1e4})):v()}//!=========== Роблю перевірку для observer
